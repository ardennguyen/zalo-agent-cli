...
- Automatic media download — one downloader shared by `listen`, `msg history`, `mcp start` and the sync commands, saving to `~/.zalo-agent-cli/accounts/<id>/media/<conversation>/` (the MCP server can override the root with `media.downloadDir`)
