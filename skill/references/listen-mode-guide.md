...
- **Records the state that only exists here** — a reaction (`reactions`, keyed on `(msgId, userId, icon)`: Zalo **accumulates**, so one person holding three different icons on one message is three reactions and all three are displayed. A reaction is named by `content.rMsg[].gMsgID`, not by the event's own `msgId`, which identifies the notification), a delivery/read receipt (`msgStatus`, forward-only: a late "delivered" cannot undo a "seen"), a board change (flags the conversation for the next `sync-boards`), and leaving or being removed from a group (`threads.leftAt`, which is what makes `conv forget --orphans` able to find it).
