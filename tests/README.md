...
```bash
npm test                      # 1118 offline tests — no Zalo session needed
...
| `unit/sync-v2-plan.test.js`            | What one `zalo-agent sync` run does: at most one phone prompt across every stage combination, one socket window, a running daemon never evicted                                      |
...
| `unit/sync-socket-rules.test.js`       | Static: `sync.js` starts a listener only in `connectListener`, and the unified run never calls an exiting `run*` body                                                                |
